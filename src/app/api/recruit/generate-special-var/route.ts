...
import { SPECIAL_VARIABLE_PROMPT, GENERATE_SPECIAL_VAR_TOOL, SpecialVariableData } from '@/prompts/character-template'
...
并根据变量值的不同范围（0-20、20-40、40-60、60-80、80-100）生成对应的分阶段人设。`
    ...
    if (!toolCall) {
      return NextResponse.json({ error: 'AI未返回特殊变量数据' }, { status: 500 })
    }

    // Extract valid JSON from response using brace matching
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : '生成特殊变量失败' },
      { status: 500 }
    )
  }
}
