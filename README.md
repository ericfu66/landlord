...
- 页面入口: `/game/group-chat`
- 主要接口:
  - `POST /api/group-chat/send` 发送群聊消息（SSE）
  - `GET /api/group-chat/history` 获取历史消息
  - `GET /api/group-chat/participants` 获取参与者
  - `POST /api/group-chat/restart` 执行上下文重启与总结
  - `GET/POST /api/group-chat/summaries` 获取与选择总结
- 运行限制:
  - 单条消息长度上限 `500` 字
  - 同一用户发送冷却 `3` 秒
  - 单次触发角色上限 `5`
